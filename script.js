...
// ===================================
// 11️⃣ جلب البيانات من API + شريط تقدم من لفل → لفل
// ===================================
function fetchData(tracker, updateStatusOnly=false){
    ...
    .then(data=>{
        ...
        // ===================================
        // حساب شريط التقدم من لفل → لفل
        // ===================================
        function getRequiredExp(level){
            return 1000 + (level * 500); // يمكن تعديل القيم حسب اللعبة
        }

        let requiredExp = getRequiredExp(info.level);
        let currentLevelBaseExp = tracker.oldLevelBaseExp || info.exp - info.exp % requiredExp;

        if(tracker.oldLevel !== info.level){
            tracker.oldLevelBaseExp = info.exp;
            currentLevelBaseExp = info.exp;
        }

        let progressExp = info.exp - currentLevelBaseExp;
        let percent = (progressExp / requiredExp) * 100;
